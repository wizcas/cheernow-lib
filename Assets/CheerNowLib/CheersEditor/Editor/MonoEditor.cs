...
[CustomEditor(typeof(MonoBehaviour), true), CanEditMultipleObjects]
    public class MonoEditor : Editor
    {
        ...
        ClickMe GetCheersAttribute (MemberInfo member)
        {
            var attrs = member.GetCustomAttributes(typeof(ClickMe), true);
            if (attrs == null || attrs.Length == 0)
                return null;
            return (ClickMe)attrs[0];
        }
        ...
        void DrawProperty (PropertyInfo prop, ClickMe attr, Obj[] targets)
        {
            ...
            if (EditorGUI.EndChangeCheck()) { // 有改动才为属性赋值
        ...
        void DrawMethod (MethodInfo method, ClickMe attr, Obj[] targets)
        {
        ...
        string GetCheersTitle (ClickMe attr, MemberInfo member)
        {
