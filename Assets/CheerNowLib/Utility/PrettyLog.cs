...
public class PrettyLog
{
    ...
    public class Provider : Attribute
    {
        ...
        public Provider (string module = "", string role = "", string moduleColorString = "white", string roleColorString = "white")
        {
            Module = module;
            Role = role;
            ModuleColorString = FormatColorString(moduleColorString);
            RoleColorString = FormatColorString(roleColorString);
        }
    ...
    private static string RetrieveCallerInfo ()
    {
        ...
        string moduleFormat = string.IsNullOrEmpty(provider.ModuleColorString) ? 
                "[{0}]" : 
                string.Format("<color={0}>[{{0}}]</color>", provider.ModuleColorString);
        string roleFormat = string.IsNullOrEmpty(provider.RoleColorString) ? 
                "{0}" : 
                string.Format("<color={0}>{{0}}</color>", provider.RoleColorString);
